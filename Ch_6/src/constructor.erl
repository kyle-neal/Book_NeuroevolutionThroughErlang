...
%% Ids for every sensor and actuator. The sensor and actuator names are used as input to the create_Sensor
...
%% vector lengths from the sensor and actuator used by the system. The Input_VL is then used to specify
%% how many weights the neurons in the input layer will need, and the Output_VL specifies how many
%% neurons are in the output layer of the NN. After appending the HiddenLayerDensites to the now known
%% number of neurons in the last layer to generate the full LayerDensities list, we use the create_NeuroLayers
%% function to generate the Neuron representing tuples. We then update the Sensor and Actuator records
%% with proper fanin and fanout ids from the freshly created Neuron tuples, composes the Cortex, and
...
%% (in the case of an actuator). It is a function that we need to define and program before it is used,
%% and the name of the function is the same as the name of the sensor or actuator it self. For example,
%% the create_Sensor/1 has specified only the rng sensor, because that is the only sensor function we've
%% finished developing. The rng function has its own vl specification,  which will determine the number
%% of weights that a neuron will need to allocate if it is to accept this sensor's output vector. The
%% same principles  apply to the create_Actuator function. Both, create_Sensor and create_Actuator
%% function, given the name of the sensor or actuator, will return a record with all the specifications
...
%% place holder Input Ids “plus”(Input_IdPs), which are tuples composed of Ids and the vector lengths
%% of the incoming signals associated with them. The proper input_idps will have a weight list in the
%% tuple instead of the vector length. Because we are only building NNs each with only a single Sensor
%% and Actuator, the IdP to the first layer is composed of the single Sensor Id with the vector length
%% of its sensory signal, likewise in the case of the Actuator. We then generate unique ids for the
...
%% held in the NIds variable. In create_NeuroLayers/7, with every iteration we generate the Output_NIds,
...
%% neurons in the next layer, the Input_IdPs and Output_Ids are the same for every neuron belonging
%% to the same layer.
...
%% cortex element. The Cortex element needs to know the Id of every Neuron, Sensors, and Actuator
%% in the NN.
